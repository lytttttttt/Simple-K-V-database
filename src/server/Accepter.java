...
public class Accepter {
    private static HashMap<SocketChannel, Handler> clientHandler = new HashMap<>(); //创建一个存放客户端通道以及对应事件处理的集合

    public Accepter(Selector selector) throws IOException {
        while (true) {    //持续历遍选择器
            ...
            while (iterator.hasNext()) {  //判断是否还有选择键
                ...
                if (selectionKey.isAcceptable()) {    //选择键是 接收 类型
                    ...
                    clientHandler.put(clientChannel, new Handler());    //将客户端通道和对应事件处理存入集合中
                    ...
                    handler.aa(clientChannel);
                } else if (selectionKey.isWritable()) {    //选择键是 可写 类型
